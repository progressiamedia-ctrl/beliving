'use client'

export const dynamic = 'force-dynamic'

import { useState, useEffect } from 'react'
import { useRouter } from 'next/navigation'
import { Header } from '@/components/Header'

type Step = 'propertyType' | 'location' | 'price' | 'occupancy' | 'goals' | 'done'

export default function HostOnboarding() {
  const router = useRouter()
  const [step, setStep] = useState<Step>('propertyType')
  const [responses, setResponses] = useState({
    propertyType: '',
    location: '',
    price: '',
    occupancy: '',
    goals: [] as string[],
  })

  useEffect(() => {
    const userId = localStorage.getItem('userId')
    if (!userId) {
      router.push('/')
    }
  }, [router])

  const handleNext = () => {
    const steps: Step[] = ['propertyType', 'location', 'price', 'occupancy', 'goals', 'done']
    const currentIndex = steps.indexOf(step)
    if (currentIndex < steps.length - 1) {
      setStep(steps[currentIndex + 1])
    } else {
      localStorage.setItem('hostOnboarding', JSON.stringify(responses))
      router.push('/host/dashboard')
    }
  }

  const handleBack = () => {
    const steps: Step[] = ['propertyType', 'location', 'price', 'occupancy', 'goals', 'done']
    const currentIndex = steps.indexOf(step)
    if (currentIndex > 0) {
      setStep(steps[currentIndex - 1])
    }
  }

  const stepNumber = ['propertyType', 'location', 'price', 'occupancy', 'goals'].indexOf(step) + 1

  return (
    <div className="min-h-screen bg-white dark:bg-black">
      <Header title="Completa tu perfil - Be Living" showThemeToggle={true} />

      <div className="flex items-center justify-center min-h-[calc(100vh-80px)] p-4">
        <div className="w-full max-w-2xl">
          {/* Progress bar */}
          <div className="mb-12" role="group" aria-label="Progreso del cuestionario">
            <div className="flex justify-between items-center mb-4">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Pregunta {stepNumber} de 5
              </p>
              <p className="text-sm font-medium text-gray-900 dark:text-white" aria-live="polite">{Math.round((stepNumber / 5) * 100)}%</p>
            </div>
            <div className="w-full bg-gray-200 dark:bg-gray-800 rounded-full h-2" role="progressbar" aria-valuenow={stepNumber} aria-valuemin={1} aria-valuemax={5}>
              <div
                className="bg-black dark:bg-white h-2 rounded-full transition-all duration-300"
                style={{ width: `${(stepNumber / 5) * 100}%` }}
              />
            </div>
          </div>

        {/* Pregunta 1: Tipo de propiedad */}
        {step === 'propertyType' && (
          <div className="space-y-8">
            <div>
              <h1 className="text-4xl font-light text-black dark:text-white mb-4">
                ¿Qué tipo de propiedad tienes?
              </h1>
              <p className="text-gray-600 dark:text-gray-400">Selecciona la categoría que mejor describe tu propiedad</p>
            </div>

            <div className="grid grid-cols-2 gap-4" role="group" aria-label="Opciones de tipo de propiedad">
              {['Villa', 'Apartment', 'House', 'Condo', 'Resort', 'Boutique Hotel'].map(
                (type) => (
                  <button
                    key={type}
                    onClick={() => setResponses({ ...responses, propertyType: type })}
                    className={`p-4 border-2 rounded-lg transition ${
                      responses.propertyType === type
                        ? 'border-black dark:border-white bg-black dark:bg-white text-white dark:text-black'
                        : 'border-gray-300 dark:border-gray-700 text-black dark:text-white hover:border-black dark:hover:border-white'
                    }`}
                    aria-pressed={responses.propertyType === type}
                    aria-label={`Seleccionar ${type} como tipo de propiedad`}
                  >
                    {type}
                  </button>
                )
              )}
            </div>
          </div>
        )}

        {/* Pregunta 2: Ubicación */}
        {step === 'location' && (
          <div className="space-y-8">
            <div>
              <h1 className="text-4xl font-light text-black dark:text-white mb-4">
                ¿Dónde está ubicada tu propiedad?
              </h1>
              <p className="text-gray-600 dark:text-gray-400">Elige la ciudad o región</p>
            </div>

            <div className="grid grid-cols-2 gap-4" role="group" aria-label="Opciones de ubicación">
              {['Dubai', 'Barcelona', 'Madrid', 'Viña del Mar', 'Bali', 'Cancun', 'Other'].map(
                (location) => (
                  <button
                    key={location}
                    onClick={() => setResponses({ ...responses, location })}
                    className={`p-4 border-2 rounded-lg transition ${
                      responses.location === location
                        ? 'border-black dark:border-white bg-black dark:bg-white text-white dark:text-black'
                        : 'border-gray-300 dark:border-gray-700 text-black dark:text-white hover:border-black dark:hover:border-white'
                    }`}
                    aria-pressed={responses.location === location}
                    aria-label={`Seleccionar ${location} como ubicación`}
                  >
                    {location}
                  </button>
                )
              )}
            </div>
          </div>
        )}

        {/* Pregunta 3: Precio promedio */}
        {step === 'price' && (
          <div className="space-y-8">
            <div>
              <h1 className="text-4xl font-light text-black dark:text-white mb-4">
                ¿Cuál es tu precio promedio por noche?
              </h1>
              <p className="text-gray-600 dark:text-gray-400">En dólares USD</p>
            </div>

            <div className="grid grid-cols-2 gap-4" role="group" aria-label="Opciones de precio">
              {['< $200', '$200-$400', '$400-$700', '> $700'].map((price) => (
                <button
                  key={price}
                  onClick={() => setResponses({ ...responses, price })}
                  className={`p-4 border-2 rounded-lg transition ${
                    responses.price === price
                      ? 'border-black dark:border-white bg-black dark:bg-white text-white dark:text-black'
                      : 'border-gray-300 dark:border-gray-700 text-black dark:text-white hover:border-black dark:hover:border-white'
                  }`}
                  aria-pressed={responses.price === price}
                  aria-label={`Seleccionar ${price} como precio promedio`}
                >
                  {price}
                </button>
              ))}
            </div>
          </div>
        )}

        {/* Pregunta 4: Ocupación */}
        {step === 'occupancy' && (
          <div className="space-y-8">
            <div>
              <h1 className="text-4xl font-light text-black dark:text-white mb-4">
                ¿Cuál es tu nivel de ocupación promedio?
              </h1>
              <p className="text-gray-600 dark:text-gray-400">Porcentaje de reservas al año</p>
            </div>

            <div className="space-y-3" role="group" aria-label="Opciones de nivel de ocupación">
              {['< 30%', '30-50%', '50-70%', '> 70%'].map((occupancy) => (
                <button
                  key={occupancy}
                  onClick={() => setResponses({ ...responses, occupancy })}
                  className={`w-full p-4 border-2 rounded-lg text-left transition ${
                    responses.occupancy === occupancy
                      ? 'border-black dark:border-white bg-black dark:bg-white text-white dark:text-black'
                      : 'border-gray-300 dark:border-gray-700 text-black dark:text-white hover:border-black dark:hover:border-white'
                  }`}
                  aria-pressed={responses.occupancy === occupancy}
                  aria-label={`Seleccionar ${occupancy} como nivel de ocupación`}
                >
                  {occupancy}
                </button>
              ))}
            </div>
          </div>
        )}

        {/* Pregunta 5: Objetivos */}
        {step === 'goals' && (
          <div className="space-y-8">
            <div>
              <h1 className="text-4xl font-light text-black dark:text-white mb-4">
                ¿Cuáles son tus objetivos?
              </h1>
              <p className="text-gray-600 dark:text-gray-400">Selecciona uno o más</p>
            </div>

            <div className="space-y-3" role="group" aria-label="Opciones de objetivos">
              {['More Bookings', 'Higher Pricing', 'International Exposure', 'Brand Building'].map(
                (goal) => (
                  <button
                    key={goal}
                    onClick={() => {
                      const isSelected = responses.goals.includes(goal)
                      setResponses({
                        ...responses,
                        goals: isSelected
                          ? responses.goals.filter((g) => g !== goal)
                          : [...responses.goals, goal],
                      })
                    }}
                    className={`w-full p-4 border-2 rounded-lg text-left transition ${
                      responses.goals.includes(goal)
                        ? 'border-black dark:border-white bg-black dark:bg-white text-white dark:text-black'
                        : 'border-gray-300 dark:border-gray-700 text-black dark:text-white hover:border-black dark:hover:border-white'
                    }`}
                    aria-pressed={responses.goals.includes(goal)}
                    aria-label={`${responses.goals.includes(goal) ? 'Deseleccionar' : 'Seleccionar'} ${goal} como objetivo`}
                  >
                    {goal}
                  </button>
                )
              )}
            </div>
          </div>
        )}

          {/* Botones */}
          <div className="flex gap-4 mt-12">
            {stepNumber > 1 && (
              <button
                onClick={handleBack}
                className="flex-1 px-6 py-3 border-2 border-gray-300 dark:border-gray-700 text-black dark:text-white rounded-lg hover:bg-gray-100 dark:hover:bg-gray-900 transition"
                aria-label="Ir a la pregunta anterior"
              >
                ← Atrás
              </button>
            )}
            <button
              onClick={handleNext}
              className="flex-1 px-6 py-3 bg-black dark:bg-white text-white dark:text-black rounded-lg hover:bg-gray-800 dark:hover:bg-gray-200 transition font-medium"
              aria-label={step === 'goals' ? 'Completar cuestionario y comenzar' : 'Ir a la siguiente pregunta'}
            >
              {step === 'goals' ? '¡Comenzar!' : 'Siguiente →'}
            </button>
          </div>
        </div>
      </div>
    </div>
  )
}
